 ...
 * Programming Assignment 1 - List ADT File
 */
...
class List {
    ...
    private class Node{
        ...
        Node(int inputData, Node prev, Node next){  // constructs a new node with all data given
    ...
    boolean equals(List L){
        ...
        while(clientFront.next != null && thisFront.next != null){  // stops iterating when the comparing nodes are null
            if(clientFront.data != thisFront.data)      // compares the data of both nodes
                return false;
    ...
    List concat(List L){
        List bothList = this.copy();    // copies this(first List) into new array
        Node CurrentNode = L.front;
        while(CurrentNode != null){
            bothList.append(CurrentNode.data);
            CurrentNode = CurrentNode.next;
        }
        return bothList;
    }
